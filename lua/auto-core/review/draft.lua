---auto-core.review.draft — the reviewer's DRAFT, one layer below the plugins
---that compose one.
---
---`auto-core.drafts` is the raw store: opaque scope keys, get/peek/dirty/
---discard. This is the domain layer over it — how a scope is built from a repo
---slug and a commit, what a finding looks like, how anchored and unanchored
---items are separated, who the reviewer is, and how the whole thing renders to
---Markdown.
---
---### Why it lives here
---
---It was `auto-finder.views.repos.authoring`, which made auto-finder the plugin
---no other plugin may depend on. That is the same trap the draft STORE was
---moved out of in ADR-0081 §2.2 — "the plugin holding it was the one no other
---plugin may depend on" — and it bit again the moment a second consumer wanted
---to open a commit for review: worktree.nvim's graph had to reach UP into
---auto-finder, inverting the family's dependency order (auto-core <- worktree
---<- auto-finder).
---
---Moving the domain layer down removes the reason to reach up. auto-core
---depends on neither worktree nor auto-finder and must not, so what moved is
---exactly what needed nothing from them: every function here requires only
---`auto-core.drafts`, `auto-core.git.worktree` and `auto-core.todo.vars`.
---
---`submit` deliberately did NOT move. It is the one function that reaches
---sideways — it needs `worktree.review` to write the pair — so it stays with
---the consumer that owns the write.
---@module 'auto-core.review.draft'
...
local function _drafts()
  ...
  if not ok or type(d) ~= "table" or type(d.get) ~= "function" then
    error("auto-core.review.draft: auto-core.drafts is required"
      .. " (auto-core >= v0.2.12)", 0)
  end
  return d
end
