...
-- WinNew: option-inheritance guard.
--
-- Vim's `:split` / `:vsplit` propagates window-local options from the
-- source window to the new window. The panel sets `winfixbuf=true` +
-- `winfixwidth=true` on itself (load-bearing — they're the panel's
-- protection contract); so a `:vsplit` off the panel produces a regular
-- editor window that has inherited both options. User-visible failure
-- modes:
...
--
-- The dbase-view at `auto-finder/views/dbase/layout.lua:139-167` already
-- works around this for ITS splits via `eventignore="all"` + explicit
-- reset. This autocmd promotes the workaround from per-consumer to
-- per-panel: every new non-floating, non-panel-marker window has the
-- two load-bearing inheritances cleared.
--
-- Why scheduled: `WinNew` fires DURING the split before the new winid is
-- settled as `nvim_get_current_win()`; `vim.schedule_wrap` defers to a
-- tick where the win is the current one and options can be inspected.
-- Why not `number`/`signcolumn`/`foldcolumn`: those are user-visible
-- preferences expected to come from globals / ftplugin, not from
-- auto-core's panel — resetting them would step on legitimate user
-- config. The two `winfix*` are the load-bearing ones.
-- Why correct-by-default: the ONLY in-tree producer of `winfixbuf=true`
-- on a non-floating window is `Panel:open` (verified 2026-05-22 against
-- the installed family). A future consumer that legitimately wants
-- `winfixbuf=true` on a fresh split must set it AFTER this scheduled
-- tick — same shape the panel uses post-split. See ADR 0027.
vim.api.nvim_create_autocmd("WinNew", {
  group = _guard_group,
  callback = vim.schedule_wrap(function()
    local w = vim.api.nvim_get_current_win()
    if not vim.api.nvim_win_is_valid(w) then return end
    -- Floats opt out — their owners control their options.
    local cfg_ok, cfg = pcall(vim.api.nvim_win_get_config, w)
    if not cfg_ok or cfg.relative ~= "" then return end
    -- Panel windows themselves keep their options — skip via the
    -- marker stamped in `Panel:_stamp`.
    local is_panel = false
    pcall(function()
      is_panel = vim.api.nvim_win_get_var(w, "auto_core_panel_name") ~= nil
    end)
    if is_panel then return end
    -- Detect whether either of the two inheritances is set BEFORE
    -- the reset; only emit a log entry when we actually unstuck
    -- something (avoids flooding the ring on every plain :split).
    local prev_wfb, prev_wfw = false, false
    pcall(function() prev_wfb = vim.wo[w].winfixbuf end)
    pcall(function() prev_wfw = vim.wo[w].winfixwidth end)
    if not (prev_wfb or prev_wfw) then return end
    pcall(vim.api.nvim_set_option_value, "winfixbuf",   false, { win = w })
    pcall(vim.api.nvim_set_option_value, "winfixwidth", false, { win = w })
    log_guard.info("WinNew — cleared inherited winfix* on non-panel window",
      { fields = {
          winid = w,
          prev_winfixbuf = prev_wfb,
          prev_winfixwidth = prev_wfw,
        } })
  end),
})
