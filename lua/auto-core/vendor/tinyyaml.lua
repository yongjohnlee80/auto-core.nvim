...
--
-- Vendored from https://github.com/peposso/lua-tinyyaml (MIT, (c) 2017
-- peposso). See ./LICENSE-tinyyaml.
--
-- LOCAL MODIFICATION: native YAML 1.1 timestamp coercion is disabled
-- in parsescalar() — search for "AUTO-CORE LOCAL MODIFICATION" in this
-- file for the exact change. All other code is unmodified upstream.
--
-- auto-core's strict-subset enforcement layer lives in
-- lua/auto-core/todo/yaml.lua and forbidden YAML constructs (anchors,
-- aliases, merge keys, explicit tags) are rejected there at the
-- source level before this parser is invoked.
-------------------------------------------------------------------------------
...
local function parsescalar(line, lines, indent)
  ...
  -- AUTO-CORE LOCAL MODIFICATION: native YAML 1.1 timestamp coercion
  -- is disabled in this vendored copy. Upstream's parsescalar() greedily
  -- parses any unquoted scalar matching `YYYY-MM-DD…` into a
  -- types.timestamp value, which silently mangles task ids like
  -- `2026-05-25-foo` into `2026-05-25T00:00:00.000`. ADR-0031's strict
  -- subset uses quoted ISO datetimes for the (few) fields that carry
  -- timestamps; all other date-shaped strings must round-trip as strings.
  -- Original two lines kept in a comment for diff visibility:
