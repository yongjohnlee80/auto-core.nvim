...
6. **Whitelisted commands only.** Executable editor actions MUST
   ...
   | `peek`             | Peek at the last N lines (default 20, max 200) of a target slot's TUI buffer. Read-only — does not switch focus or perturb buffer state. Args: `{ slot, lines? }`. (Renamed from `peep` in auto-agents.nvim v0.2.31.) |
